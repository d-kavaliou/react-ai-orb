...
var css_248z = ".orb-main{animation:var(--main-hue-animation);background-image:radial-gradient(circle at 50% 30%,var(--main-bg-start) 0,var(--main-bg-end) 70%);border-radius:50%;box-shadow:var(--shadow-color-1) 0 4px 6px 0,var(--shadow-color-2) 0 5px 10px 0,var(--shadow-color-3) 0 0 1px 0 inset,var(--shadow-color-4) 0 1px 7px 0 inset;cursor:pointer;height:var(--react-ai-orb-size);overflow:hidden;position:relative;width:var(--react-ai-orb-size)}.loc-a{position:absolute}.loc-b{left:10%}.loc-b,.loc-c{position:absolute;top:5%}.loc-c,.loc-d{left:5%}.loc-d{position:absolute;top:5%}.loc-glass{left:7%;position:absolute;top:6%}.shape-a{animation:rotateDiagonal var(--animation-rotation-speed-base) linear infinite,hueShift var(--animation-hue-speed-base) linear infinite;background-image:radial-gradient(circle at 50% 90%,var(--shape-a-start) 0,var(--shape-a-end) 70%)}.shape-a,.shape-b{border-radius:50%;height:72px;transform-style:preserve-3d;width:72px}.shape-b{animation:rotateDiagonal calc(var(--animation-rotation-speed-base) + var(--animation-rotation-speed-base)*.5) linear infinite,hueShift calc(var(--animation-hue-speed-base) + var(--animation-hue-speed-base)*.5) linear infinite;background:radial-gradient(circle at 33% 12%,var(--shape-b-start) 0,var(--shape-b-middle) 26%,var(--shape-b-end) 63%);filter:blur(2px);mix-blend-mode:soft-light;will-change:transform}.shape-c{animation:rotateDiagonal calc(var(--animation-rotation-speed-base) + var(--animation-rotation-speed-base)*1) linear infinite,hueShift calc(var(--animation-hue-speed-base) + var(--animation-hue-speed-base)*1) linear infinite;background-image:radial-gradient(circle at 31% 12%,var(--shape-c-start) 0,var(--shape-c-middle) 31%,var(--shape-c-end) 77%);mix-blend-mode:color-dodge;opacity:.65}.shape-c,.shape-d{border-radius:50%;filter:blur(1px);height:72px;transform-style:preserve-3d;width:72px}.shape-d{animation:rotateDiagonal calc(var(--animation-rotation-speed-base) + var(--animation-rotation-speed-base)*1.5) linear infinite,hueShift calc(var(--animation-hue-speed-base) + var(--animation-hue-speed-base)*1.5) linear infinite;background-image:radial-gradient(circle at 12% 80%,var(--shape-d-start) 0,var(--shape-d-middle) 31%,var(--shape-d-end) 77%);mix-blend-mode:color;opacity:.55}.glass{background:transparent;border-radius:50%;box-shadow:inset 0 -1px 6px 1px hsla(0,0%,100%,.5),inset 0 3px 4px 0 hsla(0,0%,100%,.5);height:72px;opacity:.8;width:72px}.blob-a{animation:rotateDiagonal calc(var(--animation-rotation-speed-base) + var(--animation-rotation-speed-base)*2) linear infinite,hueShift calc(var(--animation-hue-speed-base) + var(--animation-hue-speed-base)*2) linear infinite;mix-blend-mode:screen;opacity:.3}.blob-b{animation:rotateDiagonal calc(var(--animation-rotation-speed-base) + var(--animation-rotation-speed-base)*2.5) linear infinite,hueShift calc(var(--animation-hue-speed-base) + var(--animation-hue-speed-base)*2.5) linear infinite;mix-blend-mode:lighten;opacity:.8}.blob-b,.blob-shine{position:absolute;top:0}.blob-shine{fill:#fff;animation:expand 3.5s linear infinite;filter:blur(15px);mix-blend-mode:hard-light;opacity:.9;transform:scale(.4)}.shine-b{animation:expand-b 5s ease-in-out infinite;left:15%;mix-blend-mode:plus-lighter;opacity:.3}@keyframes expand{0%{transform:scale(.4)}50%{transform:scale(.1)}to{transform:scale(.4)}}@keyframes expand-b{0%{transform:scale(.6)}50%{transform:scale(.1)}to{transform:scale(.6)}}@keyframes rotateDiagonal{0%{transform:rotate3d(1,1,1,0deg)}to{transform:rotate3d(1,1,1,1turn)}}@keyframes hueShift{0%{filter:hue-rotate(0deg)}50%{filter:hue-rotate(var(--hue-rotation))}to{filter:hue-rotate(0deg)}}@keyframes circleAnimation{0%{transform:rotate(-120deg)}50%{transform:rotate(0)}to{transform:rotate(-120deg)}}@keyframes resetAnimation{0%{opacity:1}to{opacity:1}}";
...
const Orb = ({ mainBgStart = "rgb(236, 133, 255)", mainBgEnd = "rgb(49, 138, 255)", shadowColor1 = "rgba(166, 35, 248, 0)", shadowColor2 = "rgba(121, 19, 255, 0.5)", shadowColor3 = "rgba(255, 255, 255, 0.9)", shadowColor4 = "rgb(253, 164, 250)", shapeAStart = "rgb(133, 210, 255)", shapeAEnd = "rgba(115, 49, 255, 0)", shapeBStart = "rgb(254, 245, 254)", shapeBMiddle = "rgb(253, 109, 255)", shapeBEnd = "rgba(203, 56, 255, 0)", shapeCStart = "rgba(254, 254, 254, 0)", shapeCMiddle = "rgba(254, 111, 255, 0)", shapeCEnd = "#7006fe", shapeDStart = "rgba(254, 254, 254, 0)", shapeDMiddle = "rgba(142, 111, 255, 0)", shapeDEnd = "#00eeff", size = 1, animationSpeedBase = 1, animationSpeedHue = 1, hueRotation = 120, mainOrbHueAnimation = false, }) => {
    ...
    return (React__default["default"].createElement("div", { style: Object.assign({}, cssVariables) },
        React__default["default"].createElement("div", { className: "orb-main" },
            ...
            React__default["default"].createElement(SvgElements, { color1: mainBgStart, color2: mainBgEnd }))));
};
